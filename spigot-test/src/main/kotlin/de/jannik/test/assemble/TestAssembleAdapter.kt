...
class TestAssembleAdapter : AssembleAdapter {
    ...
    override fun getLines(player: Player): MutableList<String> {
        ...
        mutableList.add("§8» §7Max memory: §5${PlaceholderAPI.setPlaceholders(player, "%CLOUD_WRAPPER_MAX_MEMORY%")}")
        mutableList.add("§8» §7Used memory: §5${PlaceholderAPI.setPlaceholders(player, "%CLOUD_WRAPPER_USED_MEMORY%")}")
        mutableList.add("§8» §7Available memory: §5${PlaceholderAPI.setPlaceholders(player, "%CLOUD_WRAPPER_UNUSED_MEMORY%")}")
        //mutableList.add("")
        mutableList.add("§7Proxy information")
        mutableList.add("§8» §7Port: §5${PlaceholderAPI.setPlaceholders(player, "%CLOUD_PORT%")}")
        mutableList.add("§8» §7Name: §5${PlaceholderAPI.setPlaceholders(player, "%CLOUD_PROXY%")}")
        ...
        mutableList.add("§8» §7Priority: §5${PlaceholderAPI.setPlaceholders(player, "%CLOUD_PRIORITY%")}")
        //mutableList.add("")
        mutableList.add("§7Service information")
        mutableList.add("§8» §7Group: §5${PlaceholderAPI.setPlaceholders(player, "%CLOUD_GROUP%")}")
